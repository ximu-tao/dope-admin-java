...
public abstract class BasePayableEntity<T extends BaseEntity<T>> extends BaseEntity<T> implements PayableEntity<T>, BelongingUserEntity {
    ...
    @Column(comment = "支付渠道")
    @AutoColumn(comment = "支付渠道 wechat-微信支付 alipay-支付宝", notNull = true)
    ...
    @AutoColumn(comment = "支付流水号" )
    @Schema(hidden = true)
    protected String outTradeNo;
    
    
    @Column(comment = "支付状态" , onInsertValue = "1" )
    @AutoColumn(comment = "支付状态 1-待支付 2-已支付", notNull = true)
    protected Integer payStatus;
    
    @Column(comment = "客户端类型")
    @AutoColumn(comment = "客户端类型 mp_wechat-微信小程序 app-手机端 h5-手机网页", notNull = true)
    @NotNull(message = "客户端类型")
    @Schema( description = "客户端类型:mini=微信小程序,app=手机端" )
    protected String terminal;
    
        
    @AutoColumn(comment = "支付时间" )
