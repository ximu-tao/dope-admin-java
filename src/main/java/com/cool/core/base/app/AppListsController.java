...
public interface AppListsController<S extends BaseService<T>, T extends BaseEntity<T>> extends IController<S, T> {
    
    @TokenIgnore
    @Operation(summary = "查询所有数据", description = "")
    @PostMapping("/lists")
    default R<List<T>> lists(@Valid @RequestBody PageParams<T> pageParams) {
